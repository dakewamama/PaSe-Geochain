...
describe('GatewayClientCore issuance rejected with payload integration tests', () => {
  ...
  Object.keys(rejectionStatusToGatewayStatusMap).forEach((key: string) => {
    ...
    const payload = {
      matches: ['did:sol:3LWYN9tJSpxjB7ftJs7zcJi5Zoj7dubKau5Mz78qnf7b'],
    };
    before(() => {
    ...
    context(`with GK record state ${gkAPIState} mapping to gateway status ${expectedGatewayStatus}`, () => {
      ...
      step(`9. Expect the status to change to ${expectedGatewayStatus}`, async () => {
        testHelpers.expectRejected(expectedGatewayStatus, errorCode, payload);
      });
