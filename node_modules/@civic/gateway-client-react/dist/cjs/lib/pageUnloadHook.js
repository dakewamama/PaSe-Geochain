...
exports.usePageUnload = void 0;
const gateway_client_core_1 = require("@civic/gateway-client-core");
const react_1 = require("react");
const usePageUnload = (flowParameters, coreInst) => {
    const pageCloseEventSentRef = (0, react_1.useRef)(false); // Prevents duplicate analytics events on page close.
    ...
    const useBeforeUnload = () => {
        ...
        var _a, _b;
        const handler = (0, react_1.useCallback)(async () => {
            var _a, _b, _c, _d;
            if (!flowParameters) {
                gateway_client_core_1.logger.debug('No flowParameters. Cannot send analytics event.');
                return;
            }
            if (pageCloseEventSentRef.current) {
                gateway_client_core_1.logger.debug('Page close analytics event already sent. Not sending again.');
                return;
            }
            ...
            if ((_b = (_a = coreInst === null || coreInst === void 0 ? void 0 : coreInst.inputs) === null || _a === void 0 ? void 0 : _a.chainImplementation) === null || _b === void 0 ? void 0 : _b.pendingTransaction) {
                gateway_client_core_1.logger.debug('Sending analytics event on page close.');
                pageCloseEventSentRef.current = true;
                await (0, gateway_client_core_1.sendOnCloseAnalyticsEvent)(flowParameters, (_d = (_c = coreInst === null || coreInst === void 0 ? void 0 : coreInst.inputs) === null || _c === void 0 ? void 0 : _c.chainImplementation) === null || _d === void 0 ? void 0 : _d.pendingTransaction, navigator.sendBeacon.bind(navigator));
            }
        ...
        const visibilityStateCallback = (0, react_1.useCallback)(() => {
            if (document.visibilityState === 'hidden') {
                gateway_client_core_1.logger.debug('visibilitychange to hidden. Calling sendOnCloseAnalyticsEvent.');
                handler();
            }
        }, [handler]);
        const pageHideCallback = (0, react_1.useCallback)(() => {
            gateway_client_core_1.logger.debug('pagehide event fired. Calling abort on core.');
            handler();
        }, [handler]);
        const beforeUnloadCallback = (0, react_1.useCallback)(async (event) => {
            await handler();
            // eslint-disable-next-line no-param-reassign
            event.returnValue = ''; // Legacy method for cross-browser support
        }, [handler]);
        const removeAllListeners = (0, react_1.useCallback)(() => {
            gateway_client_core_1.logger.debug('Removing all page close listeners');
        ...
        (0, react_1.useEffect)(() => {
            if (document.visibilityState) {
                gateway_client_core_1.logger.debug('Browser supports visibilitychange event. Registering listener for it (core).');
                document.addEventListener('visibilitychange', visibilityStateCallback);
                return removeAllListeners;
            }
            if ('onpagehide' in window) {
                gateway_client_core_1.logger.debug('Browser supports pagehide event. Registering listener for it.');
                window.addEventListener('pagehide', pageHideCallback);
                return removeAllListeners;
            }
            // If all else fails, use beforeunload
            gateway_client_core_1.logger.debug('Adding beforeunload listener');
...
exports.usePageUnload = usePageUnload;
