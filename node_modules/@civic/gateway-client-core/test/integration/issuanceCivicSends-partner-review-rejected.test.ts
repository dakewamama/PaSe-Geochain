...
describe('GatewayClientCore issuance rejected partner-review civic-sends integration tests', () => {
  ...
  before(() => {
    ...
    testHelpers.stubSuccessfulFetchCalls({
      postResponsePayload: { transaction: undefined, ...inPartnerReviewGKAPIResponse },
      payer: null,
    });
  });
  ...
  step('9. Expect the GK-API to be polled', async () => {
    await sleep(2000);
    await testHelpers.expectGKApiPolling();
  });

  step('10. Simulate the partner rejecting the request and the GK-API returning a CREATED status', async () => {
    await testHelpers.prepareGKApiPollingResponse(GatekeeperAPIStatus.REJECTED, {
      status: GatekeeperAPIStatus.REJECTED,
    });
  });
  ...
  step('13. Expect the user to finish the flow from rejection', async () => {
